...
const FilterByPanel: React.FC = () => {
  ...
  const onSelect = (selectedKey: any) => {
    ...
    if (selectedKey && selectedKey.length > 0) {
      ...
      changeSelectedOrderStatus({ status: orderStatusList.find((item) => item.id == _keys[0]) });
    }
  };
  ...
  const prepareOrderFiltersForTree = orderStatusList.map((orderStatus) => ({
    key: `${orderStatus.id}`,
    title: (
      <span>
        <i className="fa-solid fa-house" />
        {orderStatus.name} <span className="tree-badge"> {orderStatus.num} </span>
      </span>
    ),
  }));
