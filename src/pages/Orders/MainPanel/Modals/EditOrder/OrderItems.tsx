...
const OrderItems: React.FC = () => {
  ...
  const handleOrderProductRemove = useCallback(
    (product) => {
      const updatedOrder = {
        ...editableOrder,
    ...
    [editableOrder, setEditableOrder],
  );
  ...
  const orderProducts = useMemo(
    ...
    [editableOrder, handleOrderItemChange, handleOrderProductRemove],
  );

  const productOptions = useMemo(() => {
    let options = productList.map((_item) => ({
      value: _item.key,
      label: `${_item.name} (${_item.stock} in stock)`,
    }));

    if (!!search) {
      options = options.filter((_item) => _item.label?.toLowerCase().includes(search?.toLowerCase()));
    }
