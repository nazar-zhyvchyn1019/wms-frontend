...
interface IAmountOffProductsModal {
  isOpen: boolean;
  onClose: () => void;
  onSave: () => void;
}

const AmountOffProductsModal: React.FC<IAmountOffProductsModal> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSave = () => {
    form.validateFields().then((values) => {
      console.log({ ...values, method, requirementValue, customerEligibility });
    });
  };
      ...
      <>
        <Form layout="vertical" form={form}>
          <Card title="Amount off products">
            ...
            {method === 'code' && (
              <Form.Item label="Discount code">
                <div style={{ display: 'flex', gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    <Form.Item name="code" rules={[{ required: true, message: 'Customers must enter this code at checkout.' }]}>
                      <Input size="small" />
                    </Form.Item>
                  </div>
            ...
            {method === 'automatic' && (
              <Form.Item name="code" label="Title">
                <Input size="small" />
              </Form.Item>
            )}
          </Card>
          <Card title="Value">
            <Form.Item label="Form Size" name="amount_type">
              <Radio.Group>
...
export default AmountOffProductsModal;
